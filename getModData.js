...
// 从API获取 PROFIT_PER_BUILDING_LEVEL 和 RETAIL_MODELING_QUALITY_WEIGHT
async function getConstants() {
    const url = 'https://www.simcompanies.com/api/v2/constants/core/';
    try {
        const response = await axios.get(url);
        const constants = response.data;
        return {
            PROFIT_PER_BUILDING_LEVEL: constants.PROFIT_PER_BUILDING_LEVEL,
            RETAIL_MODELING_QUALITY_WEIGHT: constants.RETAIL_MODELING_QUALITY_WEIGHT
        };
    } catch (error) {
        console.error('Error fetching constants:', error.message);
        return { PROFIT_PER_BUILDING_LEVEL: 0, RETAIL_MODELING_QUALITY_WEIGHT: 0 };
    }
}
    ...
    try {
        ...
        if (jsonDataString) {
            try {
                ...
                // 获取 PROFIT_PER_BUILDING_LEVEL 和 RETAIL_MODELING_QUALITY_WEIGHT 并添加到提取的数据中
                const constants = await getConstants();
                extractedData.PROFIT_PER_BUILDING_LEVEL = constants.PROFIT_PER_BUILDING_LEVEL;
                extractedData.RETAIL_MODELING_QUALITY_WEIGHT = constants.RETAIL_MODELING_QUALITY_WEIGHT;
